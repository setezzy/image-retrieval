...
def light(img):
    ...
    # gamma > 1 --- darker
    # gamma < 1 --- brighter
    return ep.adjust_gamma(img, random.uniform(0.8, 1.05))
...
def dilation(img):
    ...
    return mp.dilation(img, mp.square(2, dtype=np.uint64))


def erosion(img):
    # return greyscale morphological erosion of an image
    return mp.erosion(img, mp.square(2, dtype=np.uint64))


def draw_line(img):
    # draw a straight line across the digit
    r0 = random.randint(0, 28)
    c0 = random.randint(0, 4)
    r1 = random.randint(0, 28)
    c1 = random.randint(24, 28)
